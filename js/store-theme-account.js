...
const Account = {
  ...
  async fetch() {
    ...
    try {
      const headers = { 'Authorization': `Bearer ${state.activeKey}`, 'Content-Type': 'application/json' };
      const [balRes, profRes] = await Promise.allSettled([
      ...
      const balance     = bal?.balance ?? bal?.pollen ?? null;
      const tierBalance = bal?.tierBalance ?? null;
      const packBalance = bal?.packBalance ?? null;
      const tier        = prof?.tier ?? bal?.tier ?? null;
      const nextResetAt = prof?.nextResetAt ?? bal?.nextResetAt ?? null;

      this._cache = { balance, tierBalance, packBalance, tier, nextResetAt };
      this._cacheTs = now;
      return this._cache;
    } catch { return null; }
  },
