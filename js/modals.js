...
const Modals = {
  ...
  async renderModelPicker() {
    ...
    const bal      = data?.balance;      // total pollen (tierBalance + packBalance + crypto)
    const tierBal  = data?.tierBalance;  // daily free grant (resets daily)
    const packBal  = data?.packBalance;  // purchased pollen (never expires)
