...
const API = {
  ...
  async validateKey(key) {
    ...
    try {
      const r = await fetch(`${API_BASE}/account/balance`, {headers:{'Authorization':`Bearer ${key}`}});
      if (r.status===401||r.status===403) return false;
      return true;
    } catch {
      try {
        const r2 = await fetch(`${API_BASE}/text/hello?key=${encodeURIComponent(key)}`);
        if (r2.status===401||r2.status===403) return false;
        return true;
      } catch { return true; }
    }
  }
